...
type ChecklistStatus = 'pendente_entrada' | 'em_uso' | 'saida_pendente';

function getChecklistStatus(v: Vinculo): ChecklistStatus {
  if (!v.checklistEntradaId) return 'pendente_entrada';
  if (!v.checklistSaidaId)   return v.status === 'inativo' ? 'saida_pendente' : 'em_uso';
  return 'saida_pendente';
}
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      const unsub = subscribeToVinculosByCondutorId(currentUser.uid, (all) => {
        setVinculos(all.filter(
          (v) => v.status === 'ativo' || (v.status === 'inativo' && !v.checklistSaidaId),
        ));
        setCarregando(false);
      });
      return unsub;
